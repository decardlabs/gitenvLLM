...
CHECK_POINT_KEYWORDS = {
    ...
    "条件正确": ["30", "18", "between", "date_sub", "create_time"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="DeepSeek 编程能力自动化测试")
    parser.add_argument("--model", default=os.getenv("DEEPSEEK_MODEL", "deepseek-chat"), help="测试模型名")
    parser.add_argument("--temperature", type=float, default=float(os.getenv("DEEPSEEK_TEMPERATURE", "0.3")), help="采样温度")
    parser.add_argument("--timeout", type=float, default=float(os.getenv("DEEPSEEK_TIMEOUT", "120")), help="请求超时秒数")
    parser.add_argument("--sleep", type=float, default=float(os.getenv("DEEPSEEK_SLEEP_SECONDS", "1")), help="每个用例间隔秒数")
    parser.add_argument("--output", default=None, help="自定义报告输出文件名")
    parser.add_argument("--api-url", default=os.getenv("DEEPSEEK_API_URL", DEEPSEEK_API_URL), help="DeepSeek chat completions 接口地址")
    return parser.parse_args()

# ====================== 2. DeepSeek API 调用工具 ======================
def call_deepseek(prompt: str, model: str, api_key: str, api_url: str, temperature: float, timeout: float) -> dict:
    """调用 DeepSeek 大模型接口，返回结构化结果"""
    headers = {
...
# ====================== 4. 执行测试 & 生成报告 ======================
def run_deepseek_test(model: str, api_key: str, api_url: str, temperature: float, timeout: float, sleep_seconds: float) -> dict:
    """执行全部测试用例"""
    test_results = []
    total_score = 0
    start_time = time.time()

    print(f"🚀 开始测试 DeepSeek 编程能力（模型：{model}）")
...
def generate_report(report_data: dict, output_file: str | None = None):
    ...
    filename = output_file or f"DeepSeek编程能力测试报告_{model}_{datetime.now().strftime('%Y%m%d%H%M')}.md"

    # 控制台输出
    print("\n" + "=" * 80)
    print("📄 DeepSeek 编程能力综合测试报告")
    print("=" * 80)
    ...
    # Markdown 报告内容
    md_content = f"""# DeepSeek 编程能力综合测试报告
